...
if ($subId > 0 && !empty($subPwd)) {
  ...
  $qrynew = "SELECT r.comments2authors, r.lastModified,c.name FROM {$SQLprefix}committee c,{$SQLprefix}reports r WHERE subId=? AND r.revId = c.revId";
  $rownew = pdo_query($qrynew, array($subId));
...
<tr><td style="text-align: right;">Review:</td>
  <td>
    <textarea name="FeedBack" rows="15" cols="80" disabled="disabled">
EndMark;
while($result=$rownew->fetch()){
    $com=$result['comments2authors'];
    print $com.'
';
    $auth=$result['name'];
    $lastMo=$result['lastModified'];
    print '==== Reviewer: '.$auth.'  lastModified-Time: '.$lastMo.'
';
    print '
';
}
